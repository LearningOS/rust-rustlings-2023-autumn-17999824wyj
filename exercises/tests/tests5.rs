...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The `address` is assumed to be a valid memory address
    // pointing to a `u32` value. By dereferencing the pointer and using
    // it as a mutable reference, we can modify the value at that memory
    // address.
    let data = &mut *(address as *mut u32);
    *data = 0xAABBCCDD;
}
